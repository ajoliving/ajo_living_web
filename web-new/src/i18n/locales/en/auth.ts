/*
 * Authentication module English copy.
 * 1. Provides unified sign-in, user registration, and password reset text.
 * 2. Keeps the same field structure as the Traditional Chinese locale.
 */
export default {
  title: 'Sign in',
  subtitle: 'Sign in with your mobile, email, or username and password.',
  emailMode: 'Email',
  phoneMode: 'Mobile',
  usernameMode: 'User name',
  loginModeLabel: 'Sign-in method',
  loginGroupPhone: 'Mobile',
  loginGroupAccount: 'Mobile / Email / iSmart username',
  otpMode: 'Code',
  emailLogin: 'Sign in',
  emailRegister: 'Register',
  passwordLogin: 'Password',
  codeLogin: 'Code',
  usePasswordLogin: 'Use password login',
  useCodeLogin: 'Use code login',
  emailPasswordLogin: 'Email password',
  phonePasswordLogin: 'Phone password',
  emailCodeLogin: 'Email code',
  usernamePasswordLogin: 'User name password',
  username: 'User name',
  usernamePlaceholder: 'Enter your user name',
  accountIdentifierPlaceholder: 'Enter mobile, email, or iSmart username',
  registerUsernamePlaceholder: 'Set your user name',
  email: 'Email',
  emailPlaceholder: 'resident' + '@' + 'example.com',
  password: 'Password',
  passwordPlaceholder: 'Enter your password',
  confirmPassword: 'Confirm password',
  confirmPasswordPlaceholder: 'Enter your password again',
  emailOtp: 'Email code',
  emailOtpPlaceholder: 'Enter email verification code',
  engName: 'English name',
  engNamePlaceholder: 'Enter your English name',
  chiName: 'Chinese name',
  chiNamePlaceholder: 'Enter your Chinese name',
  gender: 'Gender',
  genderPlaceholder: 'Select gender',
  genderMale: 'Male',
  genderFemale: 'Female',
  idCard: 'ID card or document number',
  idCardPlaceholder: 'Enter ID card or document number',
  remark: 'Remark',
  remarkPlaceholder: 'Enter a remark',
  receiveEmail: 'Receive email notifications',
  displayName: 'Display name',
  displayNamePlaceholder: 'Enter your display name',
  residenceBuilding: 'Building',
  residenceBuildingPlaceholder: 'Select building',
  residenceBuildingLoading: 'Loading buildings',
  residenceFloor: 'Floor',
  residenceFloorUnit: 'Floor / Unit',
  residenceFloorPlaceholder: 'Select floor',
  residenceUnassignedFloor: 'Unassigned',
  residenceUnit: 'Unit',
  residenceUnitPlaceholder: 'Select unit',
  residenceUnitLoading: 'Loading units',
  bindAjoBuilding: 'Bind building (for AJO iSmart buildings)',
  bindAjoPlatformDescription: '*Users can also bind it in Member Centre after registration',
  missingBuildingHint: 'If your building is not listed, submit the information in Member Centre.',
  optionalFieldHint: '(Optional)',
  publisherIdentityType: 'Account type',
  accountTypePersonal: 'Personal account',
  accountTypeIndividualAgent: 'Individual agent',
  accountTypeAgencyCompany: 'Agency company',
  accountTypeAgencyCompanySubaccount: 'Agency company subaccount',
  agencyLicenseNumber: 'Licence number',
  individualAgencyLicensePlaceholder: 'For example E-123456 or S-123456',
  individualAgencyLicenseHint: 'E: Individual estate agent licence. S: Salesperson licence.',
  companyAgencyLicensePlaceholder: 'For example C-123456',
  companyAgencyLicenseHint: 'C: Estate agent company licence.',
  companyNameZh: 'Company name in Chinese',
  companyNameZhPlaceholder: 'Enter the company name in Chinese',
  companyNameEn: 'Company name in English',
  companyNameEnPlaceholder: 'Enter the company name in English',
  agencyContactName: 'Contact person',
  agencyContactNamePlaceholder: 'Enter the contact person name',
  individualAgencyLicenseUpload: 'Upload individual licence',
  companyAgencyLicenseUpload: 'Upload company licence',
  agencyNameRequired: 'Enter the required name.',
  agencyLicenseInvalid: 'Enter a valid licence number format.',
  agencyLicenseFileRequired: 'Upload the licence evidence.',
  agencyLicenseFileInvalid: 'Upload a PNG, JPG, or WEBP image up to 10MB.',
  agencyLicenseUploadFailed: 'Licence upload failed. Please try again later.',
  agencyReviewRequired: 'The platform will verify the submission. Normal access is available only after approval.',
  agencyRegisterSuccess: 'Agency details submitted. Please wait for platform approval.',
  identityOwner: 'Owner',
  identityAgent: 'Property agent',
  phone: 'Mobile number',
  phoneCountryCode: 'Country code',
  phonePlaceholder: 'For example +852 6123 4567',
  phoneSmsHint: 'Must be able to receive SMS',
  otp: 'OTP',
  otpPlaceholder: 'Enter verification code',
  requestOtp: 'Request OTP',
  submit: 'Sign in',
  registerSubmit: 'Create account',
  registrationStep: 'Registration step {step} of 2',
  registrationNext: 'Next',
  registrationBack: 'Back',
  residentLogin: 'Welcome back, AJO Living!',
  residentRegister: 'User registration',
  activeSession: 'Current account',
  securityTitle: 'Secure sign-in',
  securityDescription: 'Email verification code and password sign-in are used for member sign-in and identity checks.',
  rememberMe: 'Remember me',
  forgotPassword: 'Forgot password',
  passwordResetEyebrow: 'Account security',
  passwordResetTitle: 'Reset password',
  passwordResetDescription: 'Enter the email bound to your account and set a new password with the verification code.',
  passwordResetSendCode: 'Send email code',
  passwordResetResendCode: 'Resend code',
  passwordResetNewPassword: 'New password',
  passwordResetConfirmPassword: 'Confirm new password',
  passwordResetConfirmPlaceholder: 'Enter the new password again',
  passwordResetSubmit: 'Reset password',
  passwordResetBackToLogin: 'Back to sign in',
  passwordResetCodeSent: 'Password reset code sent.',
  passwordResetCodeRequired: 'Email verification code is required.',
  passwordResetPasswordRequired: 'Please enter a new password with at least 8 characters.',
  passwordResetConfirmRequired: 'Please enter the new password again.',
  passwordResetConfirmMismatch: 'The new passwords do not match.',
  passwordResetSuccess: 'Password reset. Please sign in again.',
  passwordResetFailed: 'Failed to reset password.',
  showPassword: 'Show password',
  hidePassword: 'Hide password',
  continueWith: 'Or continue with',
  alreadyHaveAccount: 'Already have an account?',
  dontHaveAccount: 'Don’t have an account?',
  invalidEmail: 'Please enter a valid email address.',
  invalidPhone: 'Please enter a valid mobile number, for example +852 61234567.',
  emailAlreadyRegistered: 'This email is already in use.',
  phoneAlreadyRegistered: 'This mobile number is already in use.',
  accountRequired: 'Enter a mobile number, email, or iSmart username.',
  formInvalid: 'Check the highlighted required fields.',
  requiredFields: 'Required information is missing.',
  emailOtpRequiredFields: 'A valid email and verification code are required.',
  passwordRequired: 'Password is required.',
  usernamePasswordRequired: 'A valid user name and password are required.',
  phonePasswordRequired: 'A valid phone number and password are required.',
  registerRequiredFields: 'A user name, valid phone number, and a password with at least 8 characters are required.',
  registerEnglishNameInvalid: 'Enter an English name between 2 and 120 characters.',
  idCardRequired: 'Enter an ID card or document number.',
  registerUsernameInvalid: 'Enter a user name between 2 and 120 characters without spaces.',
  registerPasswordTooShort: 'Enter a password with at least 8 characters.',
  registerPasswordMismatch: 'The passwords do not match.',
  residenceBindingRequired: 'Select a building, floor, and unit to submit the binding application.',
  otpSent: 'Verification code sent.',
  otpPreview: 'Verification code: {code}',
  signInSuccess: 'Signed in successfully.',
  registerSuccess: 'Account created. Any submitted property-binding application is subject to management-office approval.',
  signOutSuccess: 'Signed out successfully.',
  sending: 'Sending...',
  loading: 'Loading...',
  hero: {
    imageAlt: 'Hong Kong harbour residence view',
    title: 'Welcome home',
    description: 'Open your ideal community life with listings, contacts, and neighbourhood trading in one place.',
    locationLabel: 'Location',
    photoLabel: 'Photo',
    established: 'Est. 2024',
  },
};
