...
describe('SupermarketOffersPage search', () => {
  ...
  it('shows suggestions for the typed product query', async () => {
    ...
    expect(wrapper.find('.gp-search-suggestion-name').text()).toBe('花生醬 - 幼滑裝');
    expect(wrapper.find('.gp-search-suggestion-chip').text()).toBe('Meadows');
    expect(wrapper.find('.gp-search-suggestion-unit').text()).toBe('340克');
  ...
  // 4. 商品卡片必須在圖片下方顯示品牌、名稱與規格。
  it('shows brand, product, and unit details below the image', async () => {
    const wrapper = mount(Page, { global: { plugins: [createPinia(), i18n], stubs: { AppIcon: true } } });
    await flushPromises();
    ...
    expect(wrapper.find('.gp-card-name').text()).toBe('花生醬 - 幼滑裝');
