...
const validateProfileForReview = (): boolean => {
  ...
  if (profileType.value === 'company') {
    setProfileRequiredError('address_zh', 'account.agencyCompany.addressZh', form.address_zh);
    setProfileRequiredError('address_en', 'account.agencyCompany.addressEn', form.address_en);
  }
  ...
  if (profileType.value === 'company') {
    profileFieldErrors.business_registration_asset_id = form.business_registration_asset_id
      ? ''
      : t('account.agencyCompany.fieldRequired', { field: t('account.agencyCompany.businessRegistration') });
  }
        ...
        <fieldset :disabled="isReadonly || isSaving || isSubmitting">
          ...
          <template v-else>
            ...
            <div class="form-grid">
              ...
              <label :class="{ 'agency-field--error': profileFieldErrors.address_zh }"><span>{{ t('account.agencyCompany.addressZh') }} <i class="agency-required-mark" aria-hidden="true">*</i></span><input v-model.trim="form.address_zh" :aria-invalid="profileFieldErrors.address_zh ? 'true' : undefined" :aria-describedby="profileFieldErrors.address_zh ? 'agency-address-zh-error' : undefined" maxlength="300" required @input="clearProfileFieldError('address_zh')"><small v-if="profileFieldErrors.address_zh" id="agency-address-zh-error" class="agency-field-error" role="alert">{{ profileFieldErrors.address_zh }}</small></label>
              <label :class="{ 'agency-field--error': profileFieldErrors.address_en }"><span>{{ t('account.agencyCompany.addressEn') }} <i class="agency-required-mark" aria-hidden="true">*</i></span><input v-model.trim="form.address_en" :aria-invalid="profileFieldErrors.address_en ? 'true' : undefined" :aria-describedby="profileFieldErrors.address_en ? 'agency-address-en-error' : undefined" maxlength="500" required @input="clearProfileFieldError('address_en')"><small v-if="profileFieldErrors.address_en" id="agency-address-en-error" class="agency-field-error" role="alert">{{ profileFieldErrors.address_en }}</small></label>
          ...
          <div class="asset-grid">
            ...
            <label v-if="profileType === 'company'" class="asset-field" :class="{ 'agency-field--error': profileFieldErrors.business_registration_asset_id }"><span>{{ t('account.agencyCompany.businessRegistration') }} <i class="agency-required-mark" aria-hidden="true">*</i></span><img v-if="assets.business_registration" :src="assets.business_registration.url" alt=""><input type="file" accept="image/*" :aria-invalid="profileFieldErrors.business_registration_asset_id ? 'true' : undefined" :aria-describedby="profileFieldErrors.business_registration_asset_id ? 'agency-registration-error' : undefined" @change="uploadAsset($event, 'business_registration')"><small v-if="profileFieldErrors.business_registration_asset_id" id="agency-registration-error" class="agency-field-error" role="alert">{{ profileFieldErrors.business_registration_asset_id }}</small><button v-if="assets.business_registration" type="button" @click="removeAsset('business_registration')">{{ t('account.agencyCompany.removeFile') }}</button></label>
