...
- `web-new` 內凡屬 iSmart 能力的會員中心、我的大廈、門禁、支付、服務個案與副戶功能，預設都應視為 AJO 對 `/Users/yangliu/Documents/Code/hk/ajo_ismart` 已暴露接口的受控代理；上游未暴露的 API、欄位或開關，不得在前端當成可用能力設計或展示。
- 會員中心的 iSmart 資料只讀取 `ismart_account_profile` 與受控 POS 回退欄位；已由 iSmart 文檔明確列為可更新的姓名、帳戶名稱、證件、登入電話／電郵及聯絡資料，才可經 AJO 受控代理修改；用戶名稱與加入日期維持只讀。AJO 帳戶資料只展示本系統身份、狀態、權限及本地綁定，避免重複混合。
...
2026-08-20: 記錄 `web-new` 的 iSmart 會員、大廈、門禁、支付、服務個案與副戶功能只可建立在 `ajo_ismart` 倉庫已暴露接口之上，未暴露 API 的頁面能力不得直接視為可接入。
...
2026-08-19: 會員中心 iSmart ClientTbl 只按外部接口文檔展示可用登記欄位；可更新字段經 AJO `/me/ismart/profile` 受控代理提交，用戶名稱及加入日期保持只讀。
2026-08-21: 會員中心帳號管理隱藏 iSmart 帳號編號、電話、電郵、業戶名稱、帳戶名稱及證件編號，只保留業戶、聯絡與帳單資料展示；編輯接口仍可更新文檔允許的字段。
2026-08-21: 會員中心帳號管理採用縱向主區塊；iSmart 只讀資料固定拆為業戶、聯絡及帳單三個資料分區，桌面三欄、手機單欄。
2026-08-22: 會員中心 iSmart 可編輯資料固定使用獨立彈窗承載，主頁只讀展示；彈窗內容可滾動，操作區固定在底部並適配手機端。
...
2026-08-23: 超市商品卡直接進入詳情；詳情頁只在 AJO API 返回人工確認的 `sameSeries` 時，顯示口味／配方與包裝規格切換，並只提供真實存在的商品組合。
2026-08-24: 超市優惠展示試行隱藏原價；優惠力度改按目前最低價相對第二低價的差額百分比計算，列表排序與詳情門店比較沿用該指標。
