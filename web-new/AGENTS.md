...
- 手機端頂部導航採三段式：左側漢堡選單、中間固定置中的 `AJO LIVING` Logo、右側語系切換；全站固定使用白色底色與鮮橙主操作色，不提供皮膚切換。未登入時於右側補充直接進入登入頁的「登入」入口，登入後隱藏該入口並保留底部「我的」會員入口；語系切換直接使用既有繁中與 English 偏好，不另設彈窗或第二層入口。
...
2026-07-30: 對齊代理註冊頁直接提交牌照與待審資料，牌照號碼作為代理登入名稱，批准前只可進入受限審核進度頁。
