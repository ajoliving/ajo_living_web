...
- 手機端頂部導航採三段式：左側漢堡選單、中間固定置中的 `AJO LIVING` Logo、右側語系切換；桌面頂部提供可持久化的六十組主題色選擇，主題只改變品牌語意 token，白色內容面與狀態色維持不變。未登入時於右側補充直接進入登入頁的「登入」入口，登入後隱藏該入口並保留底部「我的」會員入口；語系切換直接使用既有繁中與 English 偏好，不另設彈窗或第二層入口。
...
- 樓盤會員明確點擊儲存草稿或離開時選擇儲存，前端才提交 `charge_draft=true`；首次保存預扣 600 AJO Points，草稿發布的確認框必須使用後端返回的實際未付差額，發布總費為 1,000 AJO Points。儲存並發布的內部暫存不提交此標記，已付草稿積分由後端抵扣且後續草稿保存不得重複收費。
...
- 樓盤放售會員列表只對上架中且未成交項目提供固定一個月的延期操作；延期由後端核算並扣除現行廣告套餐積分的一半。上架中樓盤放售編輯的最後操作必須先儲存修改，再以同一半價重新發布；前端須顯示成本並要求確認。服務式住宅維持既有發布規則。
...
2026-08-12: 恢復可持久化主題色選擇器，提供四十組亮橙與二十組正式替代色，並維持白色業務表面及固定狀態色。
