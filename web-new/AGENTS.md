...
- 樓盤會員明確點擊儲存草稿或離開時選擇儲存，前端才提交 `charge_draft=true` 並顯示 1,000 AJO Points 草稿費；儲存並發布的內部暫存不提交此標記，避免與發布套餐重複收費。
- 代理帳戶註冊、資料版本、審核門禁與公司子帳戶統一歸入帳戶域；樓盤發布只消費後端核准的代理資料及公開安全快照，不得自行組合或提交代理身份。
