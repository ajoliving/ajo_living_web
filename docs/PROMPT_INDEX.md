...
| Unified account login | `web-new/src/pages/account/login/login.spec.ts`, `web-new/src/pages/account/login/widgets/LoginFormPanel.spec.ts`, `http_service/internal/service/auth_identifier_service_test.go` | Single-field login UI, one-request frontend contract, email/username/Hong Kong and mainland phone classification, local-first authentication, controlled iSmart fallback, generic credential failures, and preservation of database or upstream service errors. |
...
2026-07-22: Fixed the frontend visual baseline to a white canvas and bright-orange primary actions, removing multi-skin controls from desktop and mobile navigation.
