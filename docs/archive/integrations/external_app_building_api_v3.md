...
| Auth | `POST` | `/api/v1/integration/auth/password/` | — | Change a member password after verifying the current password |
| Notification setting | `GET` / `PATCH` | `/api/v1/integration/auth/notification-settings/` | — | Read or update the building-notice email preference |
...
- endpoint: `POST /api/v1/integration/auth/password/`
- purpose: change the iSmart login password for one member
- access control: `EXTERNAL_APP_ALLOWED_IPS`; this endpoint is for trusted AJO server-to-server calls only
...
The current password must match. The new password is checked by Django's configured password validators. Password values are never returned.
...
When AJO proxies this operation, it must update its encrypted linked iSmart credential only after this endpoint succeeds. Calling this endpoint directly from a browser is not supported.

### 1.6 Building Notice Email Setting

- endpoint: `GET` / `PATCH` `/api/v1/integration/auth/notification-settings/`
- purpose: read or update `UserSettings.blg_notice_email` (接收大廈通告電郵提示)
- access control: `EXTERNAL_APP_ALLOWED_IPS`; this endpoint is for trusted AJO server-to-server calls only
...
`receive_building_notice_email` accepts JSON booleans, `0` / `1`, or standard boolean strings. The response returns the stored boolean under the same field name.
