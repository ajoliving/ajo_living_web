...
本目錄用於保存 AJO Living 架構、部署、整合、資料、開發規範與 prompt/context 文件。
...
- `archive/`: 已歸檔的產品文件、HTML 原型與歷史備份；不作 prompt/context 索引入口。

## 歸檔規則

新增文件優先按用途放入活躍目錄；歷史產品資料、原型與備份放入 `archive/`，根目錄只保留索引與少量全局說明文件。
