 ...
 * 1. Provide staff-only user listing and admin account creation APIs.
 * 2. Keep management access behind the user is_staff flag.
 */
package service
...
// 4. StaffUserCreateParams defines staff-created account input.
...
func (s *StaffService) GetStaffMe(ctx context.Context, userID int64) (*StaffUserSummary, error) {
	...
	accessByUserID, err := s.loadAccessSnapshotsByUserIDs([]model.User{*user})
	if err != nil {
		return nil, err
	}
...
func (s *StaffService) ListUsers(ctx context.Context, filters StaffUserListFilters) ([]StaffUserSummary, *model.Pagination, error) {
	...
	if memberType := strings.TrimSpace(filters.MemberType); memberType != "" {
		query = query.Where("member_type = ?", MemberTypeUser)
	}
	...
	accessByUserID, err := s.loadAccessSnapshotsByUserIDs(users)
	if err != nil {
		return nil, nil, err
	}
...
// 8. CreateUser creates a staff-managed email password account.
func (s *StaffService) CreateUser(ctx context.Context, params StaffUserCreateParams) (*StaffUserSummary, error) {
...
// 9. resolveStaffCreateCommunity loads or mirrors a POS building for staff-created users.
...
// 10. loadUserWithProfile loads a user record with profile and community data.
...
// 11. loadProfilesByUserIDs loads profile data for a batch of users.
...
// 12. loadCredentialEmailsByUserIDs loads login email data for a batch of users.
...
// 13. loadWalletBalancesByUserIDs loads current AJO Point balances for staff lists.
...
// 14. loadAccessSnapshotsByUserIDs resolves compact access views from is_staff.
func (s *StaffService) loadAccessSnapshotsByUserIDs(users []model.User) (map[int64]*AccessSnapshot, error) {
