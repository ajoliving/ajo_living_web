...
import { fetchStaffUsers } from '@/domains/management/staff-api';
...
export type DisplayAdChannel = 'property_sale' | 'serviced_apartment' | 'furniture' | '';
export type DisplayAdPlacement = 'listing_side' | '';
export type DisplayAdLayout = 'image_full' | 'image_text' | 'text_compact';
...
export interface RewardAdForm {
  ...
  targetURL: string;
  displayChannel: DisplayAdChannel;
  displayPlacement: DisplayAdPlacement;
  displayLayout: DisplayAdLayout;
  sortOrder: number;
  rewardPoints: number;
  watchSeconds: number;
...
export interface RewardAdFormErrors {
  title?: string;
  summary?: string;
  media?: string;
...
interface UseRewardAdEditorPageOptions {
  afterSave?: () => Promise<void> | void;
  fixedAdType?: RewardAdType;
}
...
// 1.1 建立指定用途的空廣告表單
export const emptyRewardAdFormByType = (adType: RewardAdType = 'reward'): RewardAdForm => ({
  ...emptyRewardAdForm(),
  adType,
  mediaType: adType === 'display' ? 'image' : 'image',
  displayChannel: '',
  displayPlacement: 'listing_side',
  rewardPoints: adType === 'display' ? 0 : 50,
  watchSeconds: adType === 'display' ? 1 : 30,
});
...
// 3. 建立廣告發布與編輯頁流程
export const useRewardAdEditorPage = (options: UseRewardAdEditorPageOptions = {}) => {
  ...
  const adDetail = ref<StaffRewardAdResponse | null>(null);
  const adForm = reactive<RewardAdForm>(emptyRewardAdForm());
  const adFormErrors = computed<RewardAdFormErrors>(() =>
    validateRewardAdForm(adForm, isEditingAd.value, t),
  );
  ...
  const loadRewardAd = async (): Promise<void> => {
    if (!taskId.value) {
      ...
      Object.assign(adForm, emptyRewardAdFormByType(options.fixedAdType));
      adDetail.value = null;
      return;
    }
  ...
  const loadRewardAdByTaskId = async (targetTaskId: string): Promise<void> => {
    ...
    loadingAd.value = true;
    try {
      const { data } = await fetchStaffRewardAd(trimmedTaskId);
      adDetail.value = data.data;
      applyRewardAdToForm(data.data, adForm);
    } catch (error) {
      feedbackStore.pushToast(readErrorMessage(error, t('marketplace.management.walletLoadAdsError')), 'error');
  ...
  const saveRewardAd = async (): Promise<void> => {
    ...
    if (Object.keys(adFormErrors.value).length > 0) {
    ...
    try {
      ...
      if (options.afterSave) {
        ...
        await router.push({ path: '/management', query: { tab: 'reward-ads' } });
      }
    } catch (error) {
      uploadingMedia.value = false;
  ...
  watch(
    ...
    (value) => {
      if (value === 'display') {
        ...
        adForm.rewardPoints = 0;
        adForm.watchSeconds = 1;
      }
    },
  );
...
export const useRewardAdListPage = () => {
  ...
  const editRewardAd = async (ad: StaffRewardAdResponse): Promise<void> => {
    await router.push({ path: '/management', query: { tab: 'reward-ad-editor', task_id: ad.task_id } });
  };
...
const applyRewardAdToForm = (ad: StaffRewardAdResponse, form: RewardAdForm): void => {
  ...
  form.displayPlacement = ad.display_placement || 'listing_side';
  form.displayLayout = ad.display_layout;
  form.sortOrder = ad.sort_order || 1;
...
const buildRewardAdPayload = (form: RewardAdForm, mediaURL: string): StaffRewardAdPayload => {
  const isDisplayAd = form.adType === 'display';
  ...
  return {
    ...
    display_channel: '',
    display_placement: isDisplayAd ? 'listing_side' : '',
    display_layout: isDisplayAd ? form.displayLayout : 'image_text',
    sort_order: isDisplayAd ? Number(form.sortOrder) : 0,
    reward_points: isDisplayAd ? 0 : Number(form.rewardPoints),
    watch_seconds: isDisplayAd ? 1 : Number(form.watchSeconds),
    total_budget: 0,
...
// 8. 驗證廣告表單欄位
const validateRewardAdForm = (
  form: RewardAdForm,
  isEditingAd: boolean,
  t: (key: string) => string,
): RewardAdFormErrors => {
  const errors: RewardAdFormErrors = {};
  const hasMedia = Boolean(form.mediaFile) || (isEditingAd && form.mediaURL.trim().length > 0);
  if (form.title.trim().length === 0) {
    errors.title = t('marketplace.management.walletAdTitleRequired');
  }
  if (!Number.isFinite(Number(form.retentionDays)) || Number(form.retentionDays) <= 0) {
    errors.retentionDays = t('marketplace.management.walletAdRetentionDaysRequired');
  }
  if (!hasMedia) {
    errors.media = t('marketplace.management.walletAdMediaRequired');
  }

  if (form.adType === 'display') {
    if (form.mediaType !== 'image') {
      errors.media = t('marketplace.management.walletAdDisplayImageOnly');
    }
    if (!['image_full', 'image_text', 'text_compact'].includes(form.displayLayout)) {
      errors.displayLayout = t('marketplace.management.walletAdDisplayLayoutRequired');
    }
    return errors;
  }

  if (form.summary.trim().length === 0) {
    errors.summary = t('marketplace.management.walletAdSummaryRequired');
  }
  if (!Number.isFinite(Number(form.rewardPoints)) || Number(form.rewardPoints) <= 0) {
    errors.rewardPoints = t('marketplace.management.walletAdRewardRequired');
  }
  if (!Number.isFinite(Number(form.watchSeconds)) || Number(form.watchSeconds) <= 0) {
    errors.watchSeconds = t('marketplace.management.walletAdWatchSecondsRequired');
  }
  return errors;
};
