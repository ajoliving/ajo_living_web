...
import type { ChatSummaryResponse } from '@/domains/communications/api';
import { fetchChatDetail, fetchChats, markChatRead } from '@/domains/communications/api';
import type { MessageResponse } from '@/domains/communications/api';
import { fetchMessages, sendMessage } from '@/domains/communications/api';
import type { ChatConversationView, ChatMessageView } from '@/domains/communications/model';
...
// 1. 管理聊天頁資料與動作
export const useFurnitureChatPage = () => {
  ...
  const loadChats = async (): Promise<void> => {
    ...
    try {
      ...
      const deepLinkedChatId = String(route.query.chat_id || '').trim();
      ...
      if (deepLinkedChatId) {
        ...
        await router.replace({ path: '/member', query: { tab: 'chat', chat_id: targetConversation.id } });
  ...
  const handleSelectChat = async (chatId: string): Promise<void> => {
    ...
    await router.replace({ path: '/member', query: { tab: 'chat', chat_id: chatId } });
  };
