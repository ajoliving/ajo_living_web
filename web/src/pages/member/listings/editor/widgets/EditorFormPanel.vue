...
<template>
  ...
  >
    ...
    <section class="editor-section">
      ...
      <div
        v-if="props.formState.visibility === 'building_only'"
        class="editor-field-grid editor-community-grid"
      >
        <div class="editor-field editor-field--wide">
          <span>{{ t('marketplace.editor.visibilityCommunityField') }}</span>
          <AppGlassSelect
            v-model="props.formState.communityId"
            :options="props.buildingOptions"
            :disabled="props.buildingsLoading"
            panel-max-height="18rem"
          />
        </div>
      </div>
