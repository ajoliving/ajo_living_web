...
import type { SecondhandListingDetailResponse } from '@/domains/marketplace/model';
...
import { resolveListingCoverImage, resolveListingStatus } from '@/shared/utils/marketplace';
...
export const useMarketplaceMyListingPreviewPage = () => {
  ...
  const listingId = computed(() => String(route.query.listing_id ?? ''));
  ...
  const openEditor = async (): Promise<void> => {
    ...
    await router.push({ path: '/member', query: { tab: 'listing-editor', listing_id: listingId.value } });
  };
  ...
  const openPublicDetail = async (): Promise<void> => {
    ...
    await router.push(`/furniture/listing/${listingId.value}`);
  };
